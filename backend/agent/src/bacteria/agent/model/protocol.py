...
and it is the reason ``bacteria.agent.model.client`` (Anthropic) and
``bacteria.agent.model.gemini_client`` (Gemini) can be interchanged by the CLI with
...
because that is what :mod:`bacteria.agent.runtime.runtime` constructs. Any
...
see :mod:`bacteria.agent.model.gemini_client` for a worked example of how much
...
:mod:`bacteria.agent.model.gemini_client`, and it is worth reading that module before
...
class ToolCall(TypedDict):
    ...
    happens in :func:`bacteria.agent.tools.execution.execute_tool_call`, which no
    ...
    Attributes:
        ...
        name: Must match a tool registered in a :class:`~bacteria.agent.tools.registry.ToolRegistry`;
