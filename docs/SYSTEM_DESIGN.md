...
**Implementation status:** built, imports cleanly, and was run for real (no `ANTHROPIC_API_KEY` set locally). Running it surfaced a genuine gap rather than a hypothetical one: `ModelClient._classify()` had never been exercised against a real Anthropic SDK exception before. A missing-credentials failure raises a raw `TypeError` from the SDK's auth resolution, before any HTTP call — that didn't match any of `_classify`'s named exception types, so it fell through to the generic `ContractError` catch-all. Fixed: added a fourth error category, `CredentialsError` (see [`src/bacteria/model/errors.py`](../src/bacteria/model/errors.py)), deliberately kept outside Part 2's asset/serving/contract three-way split — those all describe a well-formed *attempt* failing, whereas missing/invalid credentials mean the attempt was never authorized at all. `_classify()` now recognizes both the local pre-flight `TypeError` and the server-rejected `anthropic.AuthenticationError` (401) case, routing both to `CredentialsError` — non-retryable, same as `AssetError`/`ContractError`. Two new load-bearing tests in [`tests/test_model_client.py`](../tests/test_model_client.py) (`test_missing_credentials_is_not_retried`, `test_rejected_credentials_is_not_retried`); full suite now 25/25. **Update:** the CLI now also registers `add_note` and wires `cli_approve`, closing the "no real tool anywhere" gap flagged since Part 6. 4 new load-bearing tests in [`tests/test_tool_notes.py`](../tests/test_tool_notes.py); full suite now 45/45. **Update:** the CLI now also loads `.env` via `python-dotenv` before constructing `ModelClient` — the SDK only reads real process environment variables, never a `.env` file on its own, and this gap surfaced the first time credentials were actually configured for a live run.

---

## GeminiClient — a second SendsMessages implementation (outside the article's Part sequence)

Not tied to any Part — the article series is provider-agnostic by design, and this is the first real test of a decision made all the way back in Part 2: `Runtime` depends only on the structural `SendsMessages` Protocol (`send(messages, **kwargs) -> ModelResponse`), never on `ModelClient` itself, specifically so a second provider could be swapped in without touching `Runtime`. Built at the user's direct request, after confirming scope: a genuine drop-in (translating Anthropic's wire shapes that `Runtime`/the tool loop construct into Gemini's `Content`/`Part`/`FunctionDeclaration` shapes) rather than a disconnected standalone script.

### Decisions for this project

- **Verified against the real SDK before writing any code**, not guessed. Two independent `ai.google.dev` doc pages both described a plausible-looking but entirely fictitious "Interactions API" (`client.interactions.create()`, `previous_interaction_id`) — a shape suspiciously identical to OpenAI's Responses API. A third check directly against the `google-genai` GitHub source (harder to hallucinate against, since it requires quoting real code) gave the actual API: `client.models.generate_content(model=, contents=, config=)`. Every field name used in the implementation (`parameters_json_schema`, `system_instruction`, `max_output_tokens`, `function_declarations`) was then cross-checked against the installed package's own `model_fields`, not assumed. → [`src/bacteria/model/gemini_client.py`](../src/bacteria/model/gemini_client.py)
- **Reuses the existing error taxonomy** (`AssetError`/`ServingError`/`ContractError`/`CredentialsError`) rather than inventing a Gemini-specific one — the categories describe which part of "talking to a model" failed, not anything Anthropic-specific. Gemini's own `ClientError`(400-499)/`ServerError`(500-599) split gets mapped onto it, with 429 deliberately special-cased out of the generic 4xx bucket into `ServingError` (retryable), same treatment as Anthropic's `RateLimitError`.
- **A second real credentials-classification gap was found and fixed the same way as Part 2's**, by actually running the code: `genai.Client()` fails *eagerly at construction* on a missing key (a bare `ValueError`), unlike Anthropic's SDK which fails lazily on the first call. Left unhandled, this exact failure mode would have escaped `_classify()` entirely, since it never reaches `send()`'s try/except. Fixed in `__init__`.
- **Made `SendsMessages` `@runtime_checkable`** so the "this really is a drop-in" claim is asserted directly (`isinstance(client, SendsMessages)`), not just implied by duck typing. → [`src/bacteria/runtime/runtime.py`](../src/bacteria/runtime/runtime.py)
- **Anthropic's tool-call wire shapes are the actual interchange format** `Runtime` and the tool-execution loop use, not a neutral intermediate one — `GeminiClient` has to translate `{"type": "tool_use"/"tool_result", ...}` blocks (including recovering the function name for a `tool_result`, which only carries `tool_use_id`, by scanning the preceding `tool_use` block) into Gemini's `Content`/`Part` shapes. This is real translation work, not a rename — the Protocol only guarantees the *call signature* is swappable, not that the wire format flowing through it is provider-neutral.
- **A tool-call dict may carry an opaque `provider_data` field**, passed through `Runtime` unchanged and never inspected there. Added after a real, live-only failure: Gemini's function-call parts carry a `thought_signature` (an opaque token tied to the model's internal reasoning state) that must be echoed back verbatim on the next turn, or the follow-up call is rejected outright — required unconditionally, not just when thinking is enabled (a narrower "just disable thinking" fix was tried first and empirically failed). `ModelResponse`'s provider-neutral `text`/`tool_calls` shape and `Runtime._assistant_content_blocks()`'s reconstruction from it had no room for that kind of state at all. `provider_data` is deliberately generic (not `thought_signature` by name) — `ModelClient` never sets it, and nothing that doesn't care about it needs to know it exists.

**Implementation status:** built and tested (11 new load-bearing invariant tests in [`tests/test_gemini_client.py`](../tests/test_gemini_client.py), covering retry/classification parity with `ModelClient`, role translation, tool_use/tool_result translation, the `thought_signature` round-trip, the construction-time credentials fix, and the Protocol conformance check itself — full suite 57/57). **Verified against the real Gemini API end to end**, including the full tool-calling loop through `Runtime` — the `thought_signature` gap above was found and fixed exactly this way, live, after mocked tests passed clean. Not wired into the CLI — `cli.py` still hardcodes `ModelClient`; picking a provider (env var, flag, or otherwise) is a separate decision, not made here.
