...
async def _call_either(fn: Callable[..., _T | Awaitable[_T]], *args: Any) -> _T:
    ...
    # ty: ignore[invalid-return-type] # type: ignore[arg-type]
...
async def execute_tool_call(
    ...
    Args:
        ...
        approve: The gate, synchronous or not — see :data:`Approve`. Defaults to
            ...
            :func:`bacteria.agent.tools.approval.cli_approve` instead. The default is
    ...
    try:
        ...
        raise ToolExecutionError(
            f"unknown tool: {name}", reason="unknown_tool") from exc
    ...
    if not await _call_either(approve, tool_call):
        raise ToolExecutionError(
            f"tool call rejected: {name}", reason="rejected")

    try:
        output = await _call_either(tool.handler, tool_call["input"])
    except Exception as exc:
        raise ToolExecutionError(
            f"tool handler failed: {name}", reason="handler_error") from exc
