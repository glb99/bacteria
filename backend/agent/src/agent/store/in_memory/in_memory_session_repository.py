...
to a recency bound in :mod:`bacteria.agent.context.assembly` — there is no relevance
...
Not built:
    ...
    :class:`bacteria.agent.session.protocol.SessionRepository` is the complete set of
...
class in_memory_session_repository:
    ...
    **Why these methods are coroutines when none of them wait for anything.**
    ...
    def _detached(self, state: SessionState) -> SessionState:
        ...
        detached.user_memory = copy.deepcopy(
            self._user_memory.get(state.session.user_id, {}))
        return detached
    ...
    async def remember(
        ...
        if scope == USER_SCOPE:
            self._user_memory.setdefault(
                state.session.user_id, {})[key] = entry
        else:
    ...
    async def propose(
        ...
        :func:`~bacteria.agent.context.assembly.assemble_context`, so an injected
        ...
        if session_id not in self._sessions:
            raise UnknownSessionError(session_id)
        state = self._sessions[session_id]
        state.proposals[(source, key)] = MemoryEntry(
    ...
    async def activate(
        ...
        if scope == USER_SCOPE:
            self._user_memory.setdefault(state.session.user_id, {})[
                key] = activated
        else:
