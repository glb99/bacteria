...
workflows — ingesting a document, processing an upload, and eventually the ones
an agent proposes. A turn is not a workflow.
"""
...
class Handler(Generic[DataType], ABC):
    ...
    def __init__(self) -> None:
        ...
        # rather than that instance's. Shared mutable-looking state on a class
        # is a bug waiting for the first subclass that touches it directly.
    ...
    def handle(self, data: DataType) -> DataType:
        ...
        Subclasses do their own work and then call ``super().handle()`` to
