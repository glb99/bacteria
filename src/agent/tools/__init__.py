...
- :mod:`~bacteria.tools.registry` — what tools exist, and which the model is
  told about this run.
- :mod:`~bacteria.tools.approval` — whether this specific call, with these
  arguments, should happen now.
- :mod:`~bacteria.tools.execution` — actually running it, once and only once
...
:mod:`~bacteria.tools.notes` is a worked example — copy its shape when adding a
tool.
"""
