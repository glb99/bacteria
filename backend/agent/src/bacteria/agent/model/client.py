"""Anthropic-backed implementation of :class:`~bacteria.agent.model.protocol.SendsMessages`.
...
Tool execution lives in :mod:`bacteria.agent.tools.execution`, which calls into this
...
class ModelClient:
    ...
    Args:
        ...
        max_retries: Additional attempts after the first, for
            :class:`~bacteria.agent.model.errors.ServingError` only.
    ...
    async def send(
        ...
        Args:
            messages: Anthropic-shaped message dicts, as built by
                :func:`bacteria.agent.context.assembly.assemble_context` and extended
                by the runtime's tool loop.
            tools: Model-facing schemas from
                :meth:`~bacteria.agent.tools.registry.ToolRegistry.schemas_for_run`.
    ...
    def _classify(exc: Exception) -> ModelLayerError:
        ...
        :class:`~bacteria.agent.model.errors.ContractError` because it is the
