...
**Implementation status:** built and tested (10 new load-bearing invariant tests in [`tests/test_gemini_client.py`](../tests/test_gemini_client.py), covering retry/classification parity with `ModelClient`, role translation, tool_use/tool_result translation, the construction-time credentials fix, and the Protocol conformance check itself — full suite 56/56). Not wired into the CLI — `cli.py` still hardcodes `ModelClient`; picking a provider (env var, flag, or otherwise) is a separate decision, not made here. Not yet run against the real Gemini API (no Gemini API key configured) — verified only against the real SDK's types/error classes and mocked network calls, same posture `ModelClient` had before the CLI entry point proved it end-to-end.
