...
def test_each_turn_gets_a_fresh_run_id():
    store = SessionStore()
...
def test_runtime_commits_via_propose_commit_not_direct_write():
    ...
    store = SessionStore()
    session = store.create_session(user_id="u1")
    client = FakeModelClient(text="assistant reply")
    runtime = Runtime(model_client=client, session_store=store)
...
def test_runtime_calls_model_client_exactly_once_per_turn():
...
def test_second_turn_sees_prior_transcript():
