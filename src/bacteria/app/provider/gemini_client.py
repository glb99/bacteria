"""Gemini-backed implementation of :class:`~bacteria.agent.model.protocol.SendsMessages`.
...
change to :mod:`bacteria.agent.runtime.runtime`, the tool loop, or the session store
...
one (see :mod:`bacteria.agent.model.protocol`), a second provider does real
...
:mod:`~bacteria.agent.model.errors` taxonomy the Anthropic client uses. Those
...
from bacteria.agent.model.errors import CredentialsError
...
_AUTH_HINTS = ("api key", "api_key", "credential",
...
class GeminiClient:
    ...
    Args:
        ...
        max_retries: Additional attempts, :class:`~bacteria.agent.model.errors.ServingError` only.
    ...
    async def send(
        ...
        :meth:`bacteria.agent.model.client.ModelClient.send` and returns the same
        :class:`~bacteria.agent.model.protocol.ModelResponse`; the translation in
        ...
        contents = [self._to_content(message, id_to_name)
    ...
    def _to_content(message: dict[str, Any], id_to_name: dict[str, str]) -> types.Content:
        ...
        for block in content:
            ...
            if kind == "text":
                ...
                signature = (block.get("provider_data")
                             or {}).get("thought_signature")
        ...
        gemini_role = "tool" if is_tool_result else (
    ...
    def _to_model_response(response: Any) -> ModelResponse:
        ...
        :data:`~bacteria.agent.model.protocol.ToolCall` ``provider_data``, which the
