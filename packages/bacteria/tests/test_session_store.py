...
async def test_get_state_returns_a_copy_not_the_authoritative_record():
    ...
    fresh = await store.get_state(session.session_id)
    assert fresh.working_state == {}
    assert fresh.transcript == []


async def test_commit_is_the_only_way_state_actually_changes():
    store = SessionStore()
    session = await store.create_session(user_id="u1")
...
async def test_session_identity_is_independent_of_user_identity():
    ...
    """
    store = SessionStore()
    session_a = await store.create_session(user_id="u1")
    session_b = await store.create_session(user_id="u1")

    assert session_a.session_id != session_b.session_id
    assert session_a.user_id == session_b.user_id == "u1"


async def test_transcript_and_working_state_are_independently_addressable():
    ...
    await store.commit(session.session_id, working_state_updates={"a": 1})
    state = await store.get_state(session.session_id)
...
async def test_unknown_session_is_rejected():
    ...
    with pytest.raises(UnknownSessionError):
        await store.get_state("does-not-exist")


async def test_memory_writes_are_explicit_and_separate_from_commit():
    ...
    await store.commit(session.session_id, working_state_updates={"scratch": 1})
    state = await store.get_state(session.session_id)
    assert state.memory == {}

    await store.remember(session.session_id, key="pref", value="concise", reason="user said so")
    state = await store.get_state(session.session_id)
...
async def test_forget_removes_a_memory_entry():
